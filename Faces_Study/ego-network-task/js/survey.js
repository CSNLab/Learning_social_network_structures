...
(function () {
    ...
    if (userId.length < 5 || userId.length > 8 || !userId.startsWith('sub-') ||
        isNaN(sid)) {
        $('body').empty();
        return;
    }
    ...
    var config = {
        apiKey: "AIzaSyCVKpmf13TgQinWrPLrKD7bMBo8uWqGSgo",
        authDomain: "network-survey-v2.firebaseapp.com",
        databaseURL: "https://network-survey-v2.firebaseio.com",
        projectId: "network-survey-v2",
        storageBucket: "network-survey-v2.appspot.com",
        messagingSenderId: "604173172216",
        appId: "1:604173172216:web:632c30aeba13699d2cea0d"
    };
    ...
    function processData(survey) {
        ...
        $.each(survey.data, function(name, response) {
            ...
            if (question_text) {
                if (name == 'social-list') {
                    // remove real names from data of this survey
            ...
            for (var j = 0; j < norbeck_q1_json.pages.length; ++j){
                ...
                for (var i = 0; i < friendList.length; ++i) {
                    matrix_json.rows.push({
                        'value': (i).toString(),
                        'text': (' ' + friendList[i].name).slice(1) // deep copy
                    });
                }
            }
        }
        ...
        if (friendList.length < 2) {
            ...
            for (var i = 0; i < friendList.length - 1; ++i) {
                matrix_json.columns.push({
                    'name': i.toString(),
                    'title': (' ' + friendList[i].name).slice(1),
                    'cellType': 'boolean',
                    "isRequired": true
                });
                matrix_json.rows.push({
                    'value': (friendList.length - i - 1).toString(),
                    'text': (' ' + friendList[friendList.length - i - 1].name).slice(1)
                });
            }
        }
    }
