...
class Presenter(object):
    ...
    """
    def __init__(self, fullscreen=True, window=None, info_logger=None, trigger=None, background_color=(-1,-1,-1)):
        """
        ...
        """
        self.window = window if window is not None else visual.Window(fullscr=fullscreen, color=background_color)
...
class DataLogger(object):
    ...
    """
    def __init__(self, filepath, filename, log_name='', logging_info=False, overwrite_file=False):
        """
        Open file
        ...
        if not os.path.isdir(filepath):
            ...
            if overwrite_file:
                print('WARNING: overwriting file ' + filepath + filename)
            else:
