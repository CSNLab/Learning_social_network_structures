...
Example:
    ...
    data_dict = load_json('data/0_1.txt',multiple_obj=True)
    ...
    list2csv(data, 'data/wide_data.csv', col_names)
    ...
    list2csv(long_data, 'data/long_data.csv', long_cols)
"""
...
def json2csv(json_file, csv_filename, nhead_lines=3, multiple_obj=True, pkl_file=None, col_names=None, delimiter=','):
    """
    Convert json file to a csv files
    Assumptions:
        first row is subject's info
        all other rows are data
    """
    data_dict = load_json(json_file, multiple_obj=multiple_obj, pkl_file=pkl_file)
    ...
    if isinstance(data_dict,(dict)):
        ...
        subinfo_dict = data_dict[0]
        del data_dict[:nhead_lines]
        colnames = subinfo_dict.keys()
        rows = range(len(data_dict))
    ...
    for r in rows:
        ...
        out_row = []
        for c in colnames:
            if c in subinfo_dict.keys():
                out_row += [subinfo_dict[c]]
            elif c in d.keys():
                out_row += [d[c]]
            else:
                out_row += ["n/a"]
        outdata.append(out_row)
    out = [colnames] + outdata
    list2csv(out, csv_filename, col_names=col_names, delimiter=delimiter)
